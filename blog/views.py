...
def like(request,num):
    if not Like.objects.filter(post=num, user=request.user.id).exists():
        post = Post.objects.get(id=num)

        if request.POST.get('like') == '1':
            Like.objects.create(post=post, user=request.user, like = True)
        else:
            Like.objects.create(post=post, user=request.user, like = False)

        like = Like.objects.filter(post=post, like = 0)
